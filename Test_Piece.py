...
class TestKnight(unittest.TestCase):
    ...
    def test_get_valid_piece_takes_squares_occupied_by_white_and_black(self):
        ...
        game.get_piece.side_effect = lambda row, col: game.board[row][col] if 0 <= row < 8 and 0 <= col < 8 else None
        game.is_valid_piece.side_effect = lambda row, col: game.get_piece(row, col) is not None and game.get_piece(row,
                                                                                                                   col) != Player.EMPTY
        valid_takes_moves = game.get_piece(3, 4).get_valid_piece_takes(
            game)  # The get_valid_piece_takes method is called on the piece at position (3, 4) using game.get_piece(3, 4).
